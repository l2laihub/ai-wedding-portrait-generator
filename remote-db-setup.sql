-- =====================================================
-- PHOTO PACKAGES REMOTE DATABASE SETUP
-- Run this in Supabase Studio SQL Editor
-- =====================================================

-- Step 1: Create photo_packages table
CREATE TABLE IF NOT EXISTS photo_packages (
    ...
    CONSTRAINT photo_packages_category_check 
    ...
    CONSTRAINT photo_packages_images_per_generation_check 
...
CREATE TABLE IF NOT EXISTS package_pricing_tiers (
    ...
    UNIQUE(package_id, name),
    CONSTRAINT package_pricing_tiers_price_cents_check CHECK (price_cents >= 0),
    CONSTRAINT package_pricing_tiers_shoots_count_check CHECK (shoots_count > 0),
    CONSTRAINT price_consistency CHECK (original_price_cents IS NULL OR original_price_cents >= price_cents)
);
...
-- Step 8: Add 3 sample engagement themes (simplified for initial testing)
WITH engagement_pkg AS (SELECT id FROM photo_packages WHERE slug = 'engagement-portraits')
...
-- Success message
SELECT 'Photo Packages system successfully created! ✅' as status,
