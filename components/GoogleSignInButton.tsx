...
const GoogleSignInButton: React.FC<GoogleSignInButtonProps> = ({
  ...
  const variantClasses = {
    primary: 'bg-white dark:bg-gray-700 border-2 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 shadow-md hover:shadow-lg',
  ...
  const loadingIconSize = size === 'sm' ? 'w-4 h-4' : size === 'lg' ? 'w-6 h-6' : 'w-5 h-5';
  ...
  return (
    ...
    >
      {isLoading ? (
        <>
          <svg 
            className={`${loadingIconSize} animate-spin`} 
            fill="none" 
            stroke="currentColor" 
            viewBox="0 0 24 24"
            strokeWidth="2"
