-- Package Rate Limiting Integration (CORRECTED)
-- Extends existing rate limiting to work with the new package system
-- Fixed all table references and data type inconsistencies

BEGIN;

-- ==========================================
-- 1. EXTEND EXISTING TABLES (CORRECTED)
-- ==========================================

-- Add package-specific columns to existing credit transactions (FIXED references)
...
-- Add package tracking to usage analytics (FIXED references)
...
-- ==========================================
-- 2. PACKAGE-AWARE RATE LIMITING (CORRECTED)
-- ==========================================

-- Create package rate limits table (FIXED package_id reference)
CREATE TABLE IF NOT EXISTS package_rate_limits (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  package_id UUID REFERENCES photo_packages(id) ON DELETE CASCADE,  -- FIXED: was TEXT, now UUID
...
-- Create package usage tracking for rate limiting (FIXED package_id reference)
...
-- CORRECTED: Use users.role instead of non-existent structure
...
-- ==========================================
-- 5. ENHANCED FUNCTIONS (CORRECTED)
-- ==========================================

-- Function to check package rate limits (FIXED parameter types)
CREATE OR REPLACE FUNCTION check_package_rate_limit(
  p_user_identifier TEXT,
  p_package_id UUID,  -- FIXED: was TEXT, now UUID
...
-- Function to increment package usage counters (FIXED parameter types)
...
-- Enhanced function for processing package usage with credits (CORRECTED)
CREATE OR REPLACE FUNCTION process_package_usage(
  p_user_id UUID,
  p_package_id UUID,  -- FIXED: was TEXT, now UUID
  p_tier_id UUID,     -- FIXED: was TEXT, now UUID
...
DECLARE
  ...
  result JSONB;
BEGIN
  -- Get tier information (CORRECTED field reference)
  SELECT * INTO tier_info
  FROM package_pricing_tiers
  WHERE id = p_tier_id AND is_active = true;  -- FIXED: was 'enabled', now 'is_active'
  ...
  -- Calculate final credit cost (CORRECTED field reference)
  final_credit_cost := 1;  -- Default cost since tier doesn't have credit_cost field
  ...
  usage_id := track_package_usage(
    ...
    tier_info.shoots_count, p_themes_used, p_session_id, p_upload_type  -- FIXED: was 'included_generations', now 'shoots_count'
  );
  ...
  VALUES (
    ...
    'Package usage: ' || tier_info.name,  -- FIXED: was 'display_name', now 'name'
  ...
  RETURN jsonb_build_object(
    ...
    'generations_included', tier_info.shoots_count  -- FIXED: was 'included_generations', now 'shoots_count'
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- ==========================================
-- 6. SEED DEFAULT RATE LIMITS (CORRECTED)
-- ==========================================

-- Insert default rate limits for the wedding package (CORRECTED to use UUID)
DO $$
DECLARE
    wedding_package_id UUID;
BEGIN
    -- Get the actual wedding package ID
    SELECT id INTO wedding_package_id 
    FROM photo_packages 
    WHERE slug = 'wedding-portraits';
    
    IF wedding_package_id IS NOT NULL THEN
        -- Insert rate limits for the wedding package
        ...
        RAISE NOTICE 'Rate limits created for wedding package: %', wedding_package_id;
    ELSE
        RAISE NOTICE 'Wedding package not found, skipping rate limit creation';
    END IF;
END $$;
...
-- ==========================================
-- 8. UTILITY VIEWS (CORRECTED)
-- ==========================================

-- View for package rate limit overview (CORRECTED references)
...
SELECT 
  ...
  prl.enabled
FROM photo_packages p  -- FIXED: was 'packages', now 'photo_packages'
LEFT JOIN package_rate_limits prl ON prl.package_id = p.id
WHERE p.is_active = true  -- FIXED: was 'status = active', now 'is_active = true'
...
COMMENT ON FUNCTION check_package_rate_limit(TEXT, UUID, TEXT) IS 'Checks if a user can use a package based on rate limits';
COMMENT ON FUNCTION increment_package_usage(TEXT, UUID, INTEGER) IS 'Increments usage counters for rate limiting';
COMMENT ON FUNCTION process_package_usage(UUID, UUID, UUID, TEXT, TEXT[], TEXT) IS 'Processes complete package usage including credits and rate limiting';
...
DO $$
BEGIN
  RAISE NOTICE 'Package Rate Limiting Integration (CORRECTED) completed successfully!';
